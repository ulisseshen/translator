...
you might check out [Common Flutter errors][Common Flutter errors].
:::
...
[`RenderBox`][`RenderBox`] objects. Many boxes in Flutter,
...
* Those that try to be as big as possible.
  For example, the boxes used by [`Center`][`Center`] and
  [`ListView`][`ListView`].
* Those that try to be the same size as their children.
  For example, the boxes used by [`Transform`][`Transform`] and
  [`Opacity`][`Opacity`].
* Those that try to be a particular size.
  For example, the boxes used by [`Image`][`Image`] and
  [`Text`][`Text`].

Some widgets, for example [`Container`][`Container`],
vary from type to type based on their constructor arguments.
The [`Container`][`Container`] constructor defaults
...
Others, for example [`Row`][`Row`] and [`Column`][`Column`] (flex boxes)
...
[this GitHub repo][this GitHub repo].
...
[`Container`][`Container`] API documentation to understand
...
which is contained by the [`RenderView`][`RenderView`] class:
the box used by the child returned by the
application's [`build`][`build`] function is given a constraint
...
the maximum height is set to [`double.infinity`][`double.infinity`].
...
(such as [`ListView`][`ListView`] and other [`ScrollView`][`ScrollView`] subclasses).

[`ListView`][`ListView`], for example,
tries to expand to fit the space available
in its cross-direction
(perhaps it's a vertically-scrolling block and
tries to be as wide as its parent).
If you nest a vertically scrolling [`ListView`][`ListView`]
...
A flex box ([`Row`][`Row`] and [`Column`][`Column`]) behaves
...
[`Expanded`][`Expanded`] when the flex box is inside
another flex box or a scrollable;
otherwise it throws an exception.

The _cross_ direction
(width for [`Column`][`Column`] or height for [`Row`][`Row`]),
...
You can find Marcelo on [GitHub][GitHub] and [pub.dev][pub.dev].

Also, thanks to [Simon Lightfoot][Simon Lightfoot] for creating the
