...
In [Dart][Dart], variables must either be explicitly
...
For more information, see [Dart's Type System][Dart's Type System].
...
:::note
As of 2.12, Dart supports [Sound Null Safety][Sound Null Safety],
...
Dart uses [`Future`][`Future`] objects to handle this.
...
For more information, see the documentation for [async and await][async and await].
...
For more information, see [Getting started][Getting started], which
...
For more information, see the Flutter [Getting started][Getting started]
documentation.
...
For more information, see the [Flutter Widget Catalog][Flutter Widget Catalog].
...
[`StatelessWidget`][`StatelessWidget`] or [`StatefulWidget`][`StatefulWidget`].
...
class uses Dart's curly brace syntax `{ }` to indicate [named parameters][named parameters].
...
For more information, see [Adding Assets and Images in Flutter][Adding Assets and Images in Flutter].
...
[Flutter packages][Flutter packages] section of [pub.dev][pub.dev].
...
For more information, see the [Flutter Technical Overview][Flutter Technical Overview].

For more information about the core widgets from the `Widgets` package,
see [Flutter Basic Widgets][Flutter Basic Widgets],
the [Flutter Widget Catalog][Flutter Widget Catalog],
or the [Flutter Widget Index][Flutter Widget Index].
...
library, such as [`Container`][`Container`], [`Column`][`Column`],
[`Row`][`Row`], and [`Center`][`Center`].
For more information, see the [Layout Widgets][Layout Widgets] catalog.
...
[`ListView`][`ListView`] is Flutter's most commonly used scrolling widget.
The default constructor takes an explicit list of children.
[`ListView`][`ListView`] is most appropriate for a small number of widgets.
...
In Flutter, you can use the [`CustomPaint`][`CustomPaint`]
and [`CustomPainter`][`CustomPainter`] classes to draw to the canvas.
...
For example, the [`Column`][`Column`] and [`Row`][`Row`] widgets
take an array of children and align them
vertically and horizontally respectively.
A [`Container`][`Container`] widget takes a combination of
layout and styling properties, and a
[`Center`][`Center`] widget centers its child widgets.
...
For example, [`Padding`][`Padding`], [`Align`][`Align`], and [`Stack`][`Stack`].

For a complete list, see [Layout Widgets][Layout Widgets].
...
For more information, see the [`Stack`][`Stack`] class documentation.
...
[`TextStyle`][`TextStyle`] class and use it for multiple `Text` widgets.
...
rich set of [Material icons][Material icons] and [colors][colors].
...
Flutter's [Cupertino (iOS-style)][Cupertino (iOS-style)] package provides high
...
The [`Colors`][`Colors`] class provides colors
from the Material Design [color palette][color palette].
...
by defining the styling in the [`ThemeData`][`ThemeData`]
...
The [`Theme`][`Theme`] widget takes a `ThemeData` in its `data` parameter
...
use a [`StatefulWidget`][`StatefulWidget`] paired with a State object.
...
[`BuildContext`][`BuildContext`] in which the widget is inflated.

[`AboutDialog`][`AboutDialog`], [`CircleAvatar`][`CircleAvatar`], and [`Text`][`Text`] are examples
of stateless widgets that subclass [`StatelessWidget`][`StatelessWidget`].
...
* When an [`InheritedWidget`][`InheritedWidget`] it depends on, changes

### The StatefulWidget

A [`StatefulWidget`][`StatefulWidget`] is a widget that changes state.
...
[`Checkbox`][`Checkbox`], [`Radio`][`Radio`], [`Slider`][`Slider`], [`InkWell`][`InkWell`],
[`Form`][`Form`], and [`TextField`][`TextField`]
...
In Flutter, use the [`shared_preferences`][`shared_preferences`] plugin to
...
* A [`Route`][`Route`] is an abstraction for an app screen or page.
* A [`Navigator`][`Navigator`] is a widget that manages routes.
...
like [`Navigator.push`][`Navigator.push`] and [`Navigator.pop`][`Navigator.pop`].
A list of routes might be specified in the [`MaterialApp`][`MaterialApp`] widget,
...
[Limitations][Limitations] in the [navigation overview][navigation overview] page.
:::
...
To navigate to a named route, the [`Navigator.of()`][`Navigator.of()`]
...
adds the given [`Route`][`Route`] to the history of the
navigator that most tightly encloses the given [`BuildContext`][`BuildContext`],
and transitions to it. In the following example,
the [`MaterialPageRoute`][`MaterialPageRoute`] widget is a modal route that
replaces the entire screen with a platform-adaptive
transition. It takes a [`WidgetBuilder`][`WidgetBuilder`] as a required parameter.
...
The [`TickerProvider`][`TickerProvider`] is an interface implemented
by classes that can vend [`Ticker`][`Ticker`] objects.
...
[`AnimationController`][`AnimationController`]. `AnimationController`s
need a `TickerProvider` to obtain their `Ticker`.
If you are creating an AnimationController from a State,
then you can use the [`TickerProviderStateMixin`][`TickerProviderStateMixin`]
or [`SingleTickerProviderStateMixin`][`SingleTickerProviderStateMixin`]
classes to obtain a suitable `TickerProvider`.

The [`Scaffold`][`Scaffold`] widget wraps a new `TabBar` widget and
...
[Material Design][Material Design] guidelines. It also supports
...
provide a [`ElevatedButton`][`ElevatedButton`], a [`Text`][`Text`] widget,
or a list of items to display as the child to the `Drawer` widget.
In the following example, the [`ListTile`][`ListTile`]
...
a single gesture, [`PanResponder`][`PanResponder`] is used.
...
in a [`GestureDetector`][`GestureDetector`].
...
see the [GestureDetector class][GestureDetector class].
...
Flutter uses the [`dart:io`][`dart:io`] core HTTP support client.
...
[`TextField`][`TextField`] and [`TextFormField`][`TextFormField`].
...
In Flutter, use the [`TextEditingController`][`TextEditingController`]
...
This is achieved using an [`AlertDialog`][`AlertDialog`]
...
In Flutter, use the [`Form`][`Form`] widget where
[`TextFormField`][`TextFormField`] widgets along with the submit
button are passed as children.
The `TextFormField` widget has a parameter called
[`onSaved`][`onSaved`] that takes a callback and executes
...
Use the [DevTools][DevTools] suite for debugging Flutter or Dart apps.
...
In Flutter, use the [`Animation`][`Animation`]
class and the [`AnimationController`][`AnimationController`] class.
...
[`AnimationController`][`AnimationController`] object named `controller`
...
A [`Tween`][`Tween`] describes the interpolation between a
...
For this example, a [`FadeTransition`][`FadeTransition`]
...
For this example, the [`FlutterLogo`][`FlutterLogo`]
...
[`Dismissible`][`Dismissible`] widget and nest the child widgets.
...
| ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| [`Button`](https://facebook.github.io/react-native/docs/button.html)                        | [`ElevatedButton`][`ElevatedButton`]                           | A basic raised button.                                                                              |
|                                                                                           |  onPressed [required]                                                                                        | The callback when the button is tapped or otherwise activated.                                                          |
|                                                                                           | Child                                                                              | The button's label.                                                                                                      |
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`Button`](https://facebook.github.io/react-native/docs/button.html)                        | [`TextButton`][`TextButton`]                               | A basic flat button.                                                                                                         |
|                                                                                           |  onPressed [required]                                                                                        | The callback when the button is tapped or otherwise activated.                                                            |
|                                                                                           | Child                                                                              | The button's label.                                                                                                      |
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`ScrollView`](https://facebook.github.io/react-native/docs/scrollview.html)                | [`ListView`][`ListView`]                                    | A scrollable list of widgets arranged linearly.|
||        children                                                                              | 	( <Widget\> [ ])  List of child widgets to display.
||controller |[ [`ScrollController`][`ScrollController`] ] An object that can be used to control a scrollable widget.
||itemExtent|[ double ] If non-null, forces the children to have the given extent in the scroll direction.
||scroll Direction|[ [`Axis`][`Axis`] ] The axis along which the scroll view scrolls.
||                                                                                                            |                                                                                                                                        |
| [`FlatList`](https://facebook.github.io/react-native/docs/flatlist.html)                    | [`ListView.builder`][`ListView.builder`]               | The constructor for a linear array of widgets that are created on demand.
||itemBuilder [required] |[[`IndexedWidgetBuilder`][`IndexedWidgetBuilder`]] helps in building the children on demand. This callback is called only with indices greater than or equal to zero and less than the itemCount.
||itemCount |[ int ] improves the ability of the `ListView` to estimate the maximum scroll extent.
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`Image`](https://facebook.github.io/react-native/docs/image.html)                         | [`Image`][`Image`]                                           | A widget that displays an image.                                                                                                       |
...
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`Modal`](https://facebook.github.io/react-native/docs/modal.html)                          | [`ModalRoute`][`ModalRoute`]                                | A route that blocks interaction with previous routes.                                                                                  |
|                                                                                           | animation                                                                                                  | The animation that drives the route's transition and the previous route's forward transition.                                          |
|                                                                                           |                                                                                                            |                                                                                                                                        |
|  [`ActivityIndicator`](https://facebook.github.io/react-native/docs/activityindicator.html) | [`CircularProgressIndicator`][`CircularProgressIndicator`] | A widget that shows progress along a circle.                                                                                           |
...
|                                                                                           |                                                                                                            |                                                                                                                                        |
|  [`ActivityIndicator`](https://facebook.github.io/react-native/docs/activityindicator.html) | [`LinearProgressIndicator`][`LinearProgressIndicator`]     | A widget that shows progress along a line.                                                                                           |
|                                                                                           | value                                                                                                      | The value of this progress indicator.                                                                                                   |
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`RefreshControl`](https://facebook.github.io/react-native/docs/refreshcontrol.html)        | [`RefreshIndicator`][`RefreshIndicator`]                   | A widget that supports the Material "swipe to refresh" idiom.                                                                          |
...
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`View`](https://facebook.github.io/react-native/docs/view.html)                            | [`Container`][`Container`]                                  | A widget that surrounds a child widget.                                                                                                                |
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`View`](https://facebook.github.io/react-native/docs/view.html)                            | [`Column`][`Column`]                                        | A widget that displays its children in a vertical array.                                                                                              |
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`View`](https://facebook.github.io/react-native/docs/view.html)                            | [`Row`][`Row`]                                              | A widget that displays its children in a horizontal array.                                                                                            |
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`View`](https://facebook.github.io/react-native/docs/view.html)                            | [`Center`][`Center`]                                        | A widget that centers its child within itself.                                                                                                       |
|                                                                                           |                                                                                                            |                                                                                                                                        |
| [`View`](https://facebook.github.io/react-native/docs/view.html)                            | [`Padding`][`Padding`]                                      | A widget that insets its child by the given padding.                                                                                                 |
|                                                                                           | padding [required]                                                                                         | [ EdgeInsets ] The amount of space to inset the child.
|||
| [`TouchableOpacity`](https://facebook.github.io/react-native/docs/touchableopacity.html)    | [`GestureDetector`][`GestureDetector`]                      | A widget that detects gestures.                                                                                                                       |
...
|||
| [`TextInput`](https://facebook.github.io/react-native/docs/textinput.html)                | [`TextInput`][`TextInput`]                                   | The interface to the system's text input control.                                                                                           |
|                                                                                           | controller                                                                                                 | [ [`TextEditingController`][`TextEditingController`] ] used to access and modify text.
|||
| [`Text`](https://facebook.github.io/react-native/docs/text.html)                          | [`Text`][`Text`]                                            | The Text widget that displays a string of text with a single style.                                                                                                                                                                           |
|                                                                                         | data                                                                                                      | [ String ] The text to display.                                                                                                                                                                              |
|                                                                                         | textDirection                                                                                             | [ [`TextAlign`][`TextAlign`] ] The direction in which the text flows.                                                                                     |
|                                                                                         |                                                                                                           |                                                                                                                                                                                                              |
| [`Switch`](https://facebook.github.io/react-native/docs/switch.html)                      | [`Switch`][`Switch`]                                      | A material design switch.                                                                                                                                                                                    |
...
|                                                                                         |                                                                                                           |                                                                                                                                                                                                              |
| [`Slider`](https://facebook.github.io/react-native/docs/slider.html)                      | [`Slider`][`Slider`]                                      | Used to select from a range of values.                                                                                                                                                                       |
