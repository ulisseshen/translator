...
see [Flutter for UIKit developers][Flutter for UIKit developers].
:::

Flutter is a framework for building cross-platform applications
that uses the Dart programming language.
To understand some differences between programming with Dart
and programming with Swift, see [Learning Dart as a Swift Developer][Learning Dart as a Swift Developer]
and [Flutter concurrency for Swift developers][Flutter concurrency for Swift developers].
...
To learn how, see [Platform adaptations][Platform adaptations].

:::tip
To integrate Flutter code into an **existing** iOS app,
check out [Add Flutter to existing app][Add Flutter to existing app].
:::
...
see [Understanding constraints][Understanding constraints].

### Design system

Because Flutter targets multiple platforms, your app doesn't need
to conform to any design system.
Though this guide features [Material][Material] widgets,
...
- [Cupertino widgets][Cupertino widgets] that follow Apple's Human Interface Guidelines
...
custom design system, check out [Wonderous][Wonderous].
...
It's common to begin your app with a [`WidgetApp`][`WidgetApp`] class,
like [`CupertinoApp`][`CupertinoApp`].
...
Note how Flutter uses the [`Center`][`Center`] widget.
...
the [Widget catalog][Widget catalog].
...
The [`CupertinoButton`][`CupertinoButton`] class comes from the Cupertino library.
...
**Flutter** uses [`Row`][`Row`] rather than `HStack`:
...
except it swaps [`Column`][`Column`] for `Row`:
...
The previous example returned a [`ListTile`][`ListTile`] widget for each item.
...
To create a scrolling view, **Flutter** uses [`SingleChildScrollView`][`SingleChildScrollView`].
...
- Get the `BoxConstraints` object in the [`LayoutBuilder`][`LayoutBuilder`] class.
- Use the [`MediaQuery.of()`][`MediaQuery.of()`] in your build functions
  to get the size and orientation of your current app.

To learn more, check out [Creating responsive and adaptive apps][Creating responsive and adaptive apps].
...
**Flutter** manages local state using a [`StatefulWidget`][`StatefulWidget`].
...
To learn more ways to manage state, check out [State management][State management].
...
For example: To rotate a button, use the [`AnimatedRotation`][`AnimatedRotation`] class.
...
To compose a new animated widget, use the [`TweenAnimationBuilder`][`TweenAnimationBuilder`].
...
like `FooTransition`.
One example would be the [`RotationTransition`][`RotationTransition`] class.

Flutter also allows you to create a custom explicit animation using
`AnimatedWidget` or `AnimatedBuilder`.

To learn more about animations in Flutter, see [Animations overview][Animations overview].
...
1. [`CustomPaint`][`CustomPaint`] that requires a painter:
...
2. [`CustomPainter`][`CustomPainter`] that implements your algorithm to draw to the canvas.
...
use [`Navigator`][`Navigator`] with named routes.
...
use a routing package such as [go_router][go_router].

To learn more, check out [Navigation and routing][Navigation and routing].
...
In **Flutter**, use the [`url_launcher`][`url_launcher`] plugin.
...
To review a complete example, check out the [`Image`][`Image`] docs.
...
In **Flutter**, add the [video_player][video_player] plugin to your project.
...
To review a complete walkthrough, check out the [video_player example][video_player example].
